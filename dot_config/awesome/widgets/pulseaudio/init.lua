...
function widget:connect_device(device)
    ...
    if device.signals.ActivePortUpdated then
        device:connect_signal(function(this, port)
            if this.object_path == self.sink.object_path then
                self:notify(pulse.get_port(self.connection, port).Name)
            end
        end,
        "ActivePortUpdated")
    end
...
function widget:init()
    ...
    self.core:ListenForSignal("org.PulseAudio.Core1.Device.ActivePortUpdated", {})
